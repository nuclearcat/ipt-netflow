...
/*
 * Conntrack notifier API differences:
 * - < 5.15: `struct nf_ct_event_notifier` uses `.fcn` and callback item is non-const
 * - >= 5.15: uses `.ct_event` and callback item is const
 *
 * We intentionally key off `LINUX_VERSION_CODE` here because `compat_def.h` may
 * be stale/unavailable on some systems (e.g. when autogenerated probes were not
 * able to run), and we still want a working build.
 */
